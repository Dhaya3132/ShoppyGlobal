  ...
  return (
    <div className='z-50 fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center' onSubmit={handleSubmit}>
        <form className='bg-white p-10 w-96 rounded-lg'>
            ...
            <div className='mt-5'>
                {currentState === 'Sign In' && <p className='font-light text-sm'>Already have an account? <span className='text-blue-500' onClick={()=> setCurrentState('Login')} >Login</span></p>}
                {currentState === 'Login' &&<p className='font-light text-sm'>Don't have an account? <span className='text-blue-500' onClick={()=> setCurrentState('Sign In')}>Sign In</span></p>}
            </div>
        </form>
    </div>
  )
